...
query MyQuery($search: String = "%%") {
  Books(where: {_or: [{book_authors: {Author: {name: {_ilike: $search}}}}, {name: {_ilike: $search}}], approved: {_eq: true}, taken: {_eq: false}}) {
    id
    name
    type
    cover_image
...
mutation MyMutation($name: String!, $type: String!, $id: Int!, $cover_image: String, $object: [book_author_insert_input!]!) {
    delete_book_author(where: {book_id: {_eq: $id}}) {
      affected_rows
    }
    update_Books_by_pk(pk_columns: {id: $id}, _set: {name: $name, id: $id, type: $type, cover_image: $cover_image, approved: null}) {
      id
    }
